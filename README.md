...
| **户部** | 镇国财神 | 💰 | 财政统筹、成本管控、数据分析 | qwen3.5-plus |
...
- 左侧导航与页面语义固定为：**总览 / 大臣 / 任务 / 流转 / 定时 / 报表 / 配置**。
- 路由、菜单、面包屑文案必须保持同一套朝廷政务语义。
- 禁止将本项目文案替换为“店铺/商品”等电商语义。

## ✅ Git 提交与推送规范（强制）

- 本项目任何代码与文档改动，**必须完成 `git commit` 并 `git push` 到当前远端分支**。
- 禁止只在本地保留未推送改动。
- 未经明确批准，禁止 `git push --force`。
